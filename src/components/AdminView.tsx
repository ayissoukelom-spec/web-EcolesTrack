...
export default function AdminView({
  ...
  const exportParentsExcel = () => {
    const rows = sortedVisibleParents.map((parent) => ({
      ...
      'Élève associé': parent.studentFirstName && parent.studentLastName
        ? `${parent.studentLastName} ${parent.studentFirstName}`
        : '',
      'École de l’élève': parent.schoolName || parent.studentSchoolName || '',
      'Dernière connexion': parent.lastLoginAt
          ...
          >
            <div className="w-full max-h-[90vh] overflow-auto">
              ...
              <div className="space-y-3 text-sm">
                ...
                {userForm.role === 'parent' && (
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Élève associé</label>
                    <select
                    ...
                    <td className="px-3 sm:px-6 py-4 text-right space-x-2">
                      ...
                      {['super_admin', 'school_admin'].includes(userRole) && (
                        <button
                          ...
                          className="inline-flex items-center gap-2 px-3 py-2 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-lg text-indigo-700 text-xs font-semibold transition-colors"
                          title="Modifier le compte parent"
                        >
                          Modifier
                        </button>
                      )}
