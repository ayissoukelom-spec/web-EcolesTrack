...
interface BulletinsListProps {
  ...
  selectedForBatchIds: number[];
  onToggleBatchSelection: (id: number, checked: boolean) => void;
...
export default function BulletinsList({
  ...
  const allVisibleSelected = items.length > 0 && items.every((item) => selectedForBatchIds.includes(item.id));
  ...
  return (
    <section className="bg-white border border-slate-100 rounded-2xl p-4 space-y-3 min-h-[420px]">
      ...
      {!loading && !error && selectedForBatchIds.length > 0 && (
        <div className="text-xs text-indigo-700 bg-indigo-50 border border-indigo-100 rounded-xl px-3 py-2">
          {selectedForBatchIds.length} bulletin(s) selectionne(s) pour telechargement.
        </div>
      )}
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-3">
                  <input
                    type="checkbox"
                    checked={selectedForBatchIds.includes(item.id)}
                    onClick={(e) => e.stopPropagation()}
                    onChange={(e) => onToggleBatchSelection(item.id, e.target.checked)}
                    className="mt-1"
                  />
                  <div>
